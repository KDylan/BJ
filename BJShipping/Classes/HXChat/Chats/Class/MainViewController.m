...
-(void)viewWillAppear:(BOOL)animated{
    ...
    //  mainVC没有关闭
    [[NSNotificationCenter defaultCenter]postNotificationName:@"isOpenChatView" object:@YES];
...
-(void)viewInit{
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem iteamWithImageNamed:@"back" hightLightImageName:@"back" target:self action:@selector(closeChatAction)];
    
//    UIButton *backButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 15, 21)];
//    // backButton.accessibilityIdentifier = @"back";
//    [backButton setImage:[UIImage imageNamed:@"back.png"] forState:UIControlStateNormal];
//    [backButton addTarget:self action:@selector(closeChatAction) forControlEvents:UIControlEventTouchUpInside];
...
-(void)closeChatAction
{
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        //发送有信消息广播
        [[NSNotificationCenter defaultCenter] postNotificationName:kHaveUnreadAtMessage object:nil];
        //  mainVC关闭了
        [[NSNotificationCenter defaultCenter]postNotificationName:@"isOpenChatView" object:@NO];
    }];
}
...
- (void)setupSubviews
{
    ...
    _chatListVC.tabBarItem = [[UITabBarItem alloc] initWithTitle:@"调度"                                                           image:[UIImage imageNamed:@"tabbar_chats"]
    ...
    _mapLocationVC = [[MapViewController alloc] init];
    _mapLocationVC.tabBarItem.title= @"位置";
    _mapLocationVC.tabBarItem.image= [UIImage imageNamed:@"radio_but_location"];
    _mapLocationVC.tabBarItem.tag = 1;
    _mapLocationVC.tabBarItem.accessibilityIdentifier = @"conversation";
    [self unSelectedTapTabBarItems:_mapLocationVC.tabBarItem];
    [self selectedTapTabBarItems:_mapLocationVC.tabBarItem];
    
    self.viewControllers = @[_chatListVC,_mapLocationVC];
    [self selectedTapTabBarItems:_chatListVC.tabBarItem];
}
...
-(void)ConfirmScheduleShip:(NSString *)ship_id stID:(NSString *)st_id{
    ...
    if (st_id) {
        ...
        //ShowHelper.toast("调度已确认，无需重复操作");
        NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
        
        NSString *val = [defaults objectForKey:st_id];
        
        if (val) {
            
            [[AlertViewHelper getInstance] show:@"提示" msg:@"调度已确认，无需重复操作"];
            
        }else{
            
            UIView *view = [[UIApplication sharedApplication].delegate window];
            
            [[AlertViewHelper getInstance] show:@"提示" msg:@"您确定调度吗？" rightBlock:^{
                
                [self showHudInView:view hint:@"确认调度中,请稍候..."];
                
                NSString *msg = [NSString stringWithFormat:@"shipId=%@&stId=%@" , ship_id , st_id];
                
                ConfirmScheduleShipApi *api = [[ConfirmScheduleShipApi alloc] initWithMsg:msg];
                [api startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
                    [self hideHud];
                    if (request.responseString) {
                       
                        NSDictionary *dic = [EdgeJsonHelper dictionaryWithJsonString:request.responseString];
                        
                        BOOL success = [dic objectForKey:@"success"];
                        
                        if (success) {
                                                       
                            [defaults setObject:st_id forKey:st_id];
                            [defaults synchronize];
                            
                            [self showHint:@"调度确认成功"];
                        }
                    }

...
//                                                                      pageSize:10
//                                                                    complation:^(EMCursorResult *aResult, EMError *aError{[super tableViewDidTriggerHeaderRefresh];}];
...
- (void)_setupBarButtonItem
{
    ...
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem iteamWithImageNamed:@"back.png" hightLightImageName:@"back.png" target:self action:@selector(backAction)];
...
/**
 ...
 @param messageModel sender
 */
- (void)touchTextViewMessageModel:(id<IMessageModel>)messageModel{
    
    EMMessage *message = messageModel.message;
    NSString *CommandType = [message.ext objectForKey:@"CommandType"];
    //发起调度请求
    if (CommandType && [CommandType isEqualToString:@"2"]) {
        
        NSString *st_id = [message.ext objectForKey:@"st_id"];
        NSString *ship_id = [message.ext objectForKey:@"ship_id"];
        
        [[MainViewController shareInstance] ConfirmScheduleShip:ship_id stID:st_id];
    }

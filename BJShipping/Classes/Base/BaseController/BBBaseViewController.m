...
-(void)viewDidLoad
{
    ...
    [[UITableView appearance] setBackgroundColor:UIColorFromRGB(DefaultBackgroundColor)];
...
- (void)netStatusChange:(NSNotification *)noti{
    ...
    switch (self.hostReach.currentReachabilityStatus) {
        case NotReachable://当前网络连接失败，请查看设置
            [self showHint:@"当前网络不可用,请检查网络设置"];
            
//            [MBProgressHUD showInfo:@"当前网络不可用，请检查网络设置" ToView:self.view];
            break;
...
-(BOOL)checkNetWorkAndShowHUD{
    BOOL flag = [self hasNetWork];
    if (!flag) {
        [self showHint:@"当前网络不可用,请检查网络设置"];
